...
pub(crate) fn left_joint(border: BorderType, joint: Joint) -> &'static str {
    use ratatui::widgets::BorderType::*;
    ...
    match (border, joint) {
        ...
        (QuadrantInside, _) => QUADRANT_TOP_LEFT_TOP_RIGHT_BOTTOM_RIGHT,
        (QuadrantOutside, Joint::In(_) | Joint::Through(_)) => {
            QUADRANT_TOP_LEFT_TOP_RIGHT_BOTTOM_LEFT
        }
        (QuadrantOutside, _) => QUADRANT_LEFT_HALF,
